...
if lang == nil then
    print('^5[Koper_logs] ^1Error: Could not load language file. Make sure you didn\'t make a typo.^0')
    return StopResource(GetCurrentResourceName())
end

if cfgFile == nil then
    print('^5[Koper_logs] ^1Error: Could not load config file. Make sure you didn\'t make a typo.^0')
    return StopResource(GetCurrentResourceName())
end
	...
	if args.url ~= "" and args.url ~= nil and args.url ~= 'DISCORD_WEBHOOK' then
		exports['screenshot-basic']:requestScreenshotUpload(args.url, 'files[]', function(data)
			...
			if resp.attachments then
				...
				TriggerServerEvent('Prefech:Koper_logs:Debug', 'Code: SB1005')
			end
		end)
	else
		TriggerServerEvent('Prefech:Koper_logs:Debug', 'Code: SB1010')
	end
end)
		...
		elseif not IsPedShooting(playerped) and fireCount ~= 0 and timeout ~= 0 then
			...
			if fireCount ~= 0 and timeout == 0 then
				if not ClientWeapons.WeaponNames[tostring(fireWeapon)] then
					...
					TriggerServerEvent('Prefech:Koper_logs:Debug', 'GF1001')
					return
				end
...
exports('discord', function(message, id, id2, color, channel)
	...
	TriggerServerEvent('Prefech:Koper_logs:Debug', 'Client Old Export from '..resource)
end)

exports('createLog', function(args)
	TriggerServerEvent('Prefech:ClientDiscord', args)
	local resource = GetInvokingResource()
	TriggerServerEvent('Prefech:Koper_logs:Debug', 'Client New Export from '..resource)
end)
...
if type(eventsFile) == "table" then
	for k,v in pairs(eventsFile) do
		if not v.Server then
			TriggerServerEvent('Prefech:Koper_logs:Debug', 'Added Client Event Log: '..v.Event)
